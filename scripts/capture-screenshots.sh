...
if [[ -n "$WATCH_UDID" ]]; then
  ...
  if [[ -n "$WATCH_APP" ]]; then
    ...
    # App Store Connect's Series 10 slot is 416x496, which is the native
    # geometry of the leased Series 11 46 mm simulator face.
    sips -z 496 416 "$RAW/06-watch.png" --out "$RAW/06-watch.png" >/dev/null
